...
describe 'gen_autosign_token' do
  ...
  context 'raises an error given incorrect parameters' do
    ...
    it { is_expected.to run.with_params('hostname.example.com', 'not_an_integer').and_raise_error(Puppet::ParseError) }
    it { is_expected.to run.with_params('hostname.example.com', 3600, 'invalid_third_parameter').and_raise_error(Puppet::ParseError) }
  end
end
