...
# This vagrant environment provides a clean slate to develop or test the autosign
...
Vagrant.configure("2") do |config|
  ...
  config.vm.synced_folder ".", "/etc/puppet/modules/autosign"
...
#    inline: "puppet module install puppetlabs-apt"
  ...
  nodes = 2
  domain = 'example.com'
  ['puppetlabs/centos-7.0-64-nocm','puppetlabs/centos-7.0-64-puppet','puppetlabs/centos-7.0-64-puppet-enterprise','puppetlabs/centos-6.6-64-puppet','puppetlabs/debian-7.8-64-puppet'].each do |box|
    platform = box.gsub('/','-')
    (1..nodes).each do |n|
      config.vm.define "#{platform}-#{n}" do |node|
        node.vm.box = box
        node.vm.hostname = "#{platform.gsub('.','-')}-#{n}.#{domain}"
